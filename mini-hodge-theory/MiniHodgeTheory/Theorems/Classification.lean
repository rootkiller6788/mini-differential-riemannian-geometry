/-
# MiniHodgeTheory: Theorems -- Classification -- L4/L8
Hodge structures, period mappings, classification results.
-/
import MiniHodgeTheory.Core.Basic
import MiniHodgeTheory.Properties.ClassificationData
namespace MiniHodgeTheory

structure HodgeStructure (w : Nat) where
  dimension : Nat
  decomposition : String
  weight : Nat := w
  conjugation : String

structure PolarizedHodgeStructure (w : Nat) extends HodgeStructure w where
  polarization : String
  hodge_riemann : String

structure VariationOfHodgeStructure where
  base_manifold : String
  fiber_hodge : String
  griffiths_transversality : String

def hodgeConjectureStmt : String :=
  "For X smooth projective complex variety, Hdg^p(X) = H^{2p}(X,Q) cap H^{p,p}(X) is generated by algebraic cycle classes. Status: Open Clay Millennium Problem."

def hodgeConjectureCases : List String := [
  "p=0: trivial",
  "p=1: Lefschetz (1,1)-theorem",
  "Abelian varieties (Murre, Hazama, Abdulali)"
]

def genHodgeConjecture : String :=
  "N^p H^k(X,Q) = H^k(X,Q) cap F^p H^k(X,C) (Grothendieck)."

def kodairaClass : List String := [
  "Rational surfaces: h^{1,0}=0, h^{2,0}=0",
  "K3 surfaces: h^{1,0}=0, h^{2,0}=1, h^{1,1}=20",
  "Enriques surfaces: h^{1,1}=10",
  "Abelian surfaces: h^{1,0}=2",
  "General type: h^{2,0}>0"
]

def calabiYauThm : String :=
  "For compact Kaehler with c_1=0, unique Ricci-flat metric in each Kaehler class (Yau 1978)."

def TorelliThm : String :=
  "A K3 surface is determined by its Hodge structure on H^2(X,Z)."

#eval "---- Theorems.Classification ----"
#eval s!"Hodge Conjecture: {hodgeConjectureStmt}"
#eval calabiYauThm
