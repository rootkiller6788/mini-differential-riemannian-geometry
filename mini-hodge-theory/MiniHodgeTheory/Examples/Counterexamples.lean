/-
# MiniHodgeTheory: Examples -- Counterexamples -- L6
Pathological cases: where Hodge theory requires extra conditions.
-/
import MiniHodgeTheory.Core.Basic
namespace MiniHodgeTheory

#eval "---- Counterexample 1: Int^n (non-compact) ----"
#eval "Harmonic forms need not be L^2-integrable"
#eval "ker(Delta) may be infinite-dimensional"
#eval "On R, f(x)=x is harmonic (f''=0) but not in L^2(R)"
#eval "Hodge decomposition requires compactness for finite-dim ker"

#eval "---- Counterexample 2: RP^2 (non-orientable) ----"
#eval "H^2(RP^2;R)=0, but H^2(RP^2;Z/2)=Z/2"
#eval "Poincare duality needs Z/2 coefficients"
#eval "b_0=1, b_1=0, b_2=0 over R; all 1 over Z/2"

#eval "---- Counterexample 3: Hopf surface S^1xS^3 (non-Kaehler) ----"
#eval "Compact complex manifold, not Kaehler"
#eval "b_1=1 is odd, but any Kaehler mfd has even b_1"
#eval "dd^c lemma fails: exists d-exact and d^c-closed but not dd^c-exact"

#eval "---- Counterexample 4: Non-smooth metric ----"
#eval "If the metric is only C^0, elliptic regularity fails"
#eval "Harmonic forms may not be smooth"
#eval "The Hodge decomposition may not give smooth representatives"

#eval "---- Counterexample 5: Iwasawa manifold (non-Kaehler complex) ----"
#eval "H^1_dR = C^4, but H^{1,0}+H^{0,1} = C^3"
#eval "Frolicher spectral sequence does not degenerate at E_1"
#eval "H^k_dR .neq. bigoplus H^{p,q} in general (without Kaehler)"

#eval "---- Counterexample 6: Integral Hodge conjecture fails ----"
#eval "Atiyah-Hirzebruch: integral Hodge conjecture is false"
#eval "Torsion classes cause counterexamples"
#eval "But Hodge conjecture over Q remains open (Clay Millennium Prize)"

#eval "---- All 6 counterexamples documented ----"
