/-
# MiniHodgeTheory: Bridges -- To Topology -- L7
Betti numbers, signature, index theorems via Hodge theory.
-/
import MiniHodgeTheory.Properties.Invariants
namespace MiniHodgeTheory

def topologyBridge : List String := [
  "b_k = dim(harmonic k-forms) via Hodge isomorphism",
  "chi = sum_k (-1)^k b_k = alternating sum of harmonic dims",
  "Signature via Hodge numbers: sigma = sum (-1)^p h^{p,q}",
  "Hirzebruch signature: sigma = integral L-genus",
  "Gauss-Bonnet-Chern: chi = integral Pfaffian",
  "Poincare duality: H^k .cong. H^{n-k} via Hodge star",
  "Intersection form on H^{2k} nondegenerate",
  "Hodge index: sign(H^{1,1})=(1,h^{1,1}-1) for Kaehler surfaces"
]

#eval "---- Bridges.ToTopology ----"
#eval s!"Topology connections: {topologyBridge.length}"
