/-
# MiniHodgeTheory: Bridges -- To Geometry -- L7/L8
Kaehler manifolds, Calabi-Yau, mirror symmetry.
-/
import MiniHodgeTheory.Theorems.Classification
namespace MiniHodgeTheory

def geometryBridge : List String := [
  "Kaehler manifolds: Hodge decomposition respects (p,q) bigrading",
  "Calabi-Yau: h^{n,0}=1, Ricci-flat metrics",
  "Mirror symmetry: h^{p,q}(X) = h^{n-p,q}(X^mirror)",
  "Special Lagrangian submanifolds in CY",
  "G_2 and Spin(7): exceptional holonomy Hodge theory",
  "Hyperkaehler: three complex structures I,J,K with IJ=K",
  "Twistor spaces: S^2 family of complex structures",
  "Kuranishi space via Hodge theory of tangent bundle"
]

#eval "---- Bridges.ToGeometry ----"
#eval s!"Geometry connections: {geometryBridge.length}"
