/-
# MiniHodgeTheory: Properties -- Preservation -- L5
Poincare duality, Lefschetz decomposition.
-/
...
def poincareDuality : String :=
  "H^k(M) ~ H^{n-k}(M) via Hodge star: star : H^k -> H^{n-k}"

def hardLefschetz : String :=
  "L^k : H^{n-k} -> H^{n+k} is an isomorphism on compact Kaehler manifolds."

def lefschetzDecomposition : String :=
  "H^k = bigoplus_{j>=0} L^j P^{k-2j} (primitive decomposition)"

def hodgeIndexTheorem : String :=
  "sign(H^{1,1}) = (1, h^{1,1}-1) for Kaehler surfaces."

#eval "---- Properties.Preservation ----"
#eval poincareDuality
#eval hardLefschetz
