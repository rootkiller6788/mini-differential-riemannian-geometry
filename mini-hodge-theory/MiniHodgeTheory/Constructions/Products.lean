/-
# MiniHodgeTheory: Constructions -- Products -- L3
Direct sums and product formulas.
-/
import MiniHodgeTheory.Core.Basic
namespace MiniHodgeTheory

def directSum (v w : RealVec) : RealVec := v ++ w

def kunnethFormula : String :=
  "H^k(X x Y) = sum_{p+q=k} H^p(X) x H^q(Y)"

def hodgeProductFormula : String :=
  "h^{p,q}(X x Y) = sum_{r+s=p,u+v=q} h^{r,u}(X) * h^{s,v}(Y)"

#eval "---- Constructions.Products ----"
#eval kunnethFormula


def bettiProductFormula : String := "b_k(X x Y) = sum_{p+q=k} b_p(X) * b_q(Y)."
def eulerProductFormula : String := "chi(X x Y) = chi(X) * chi(Y)."

def directSumInnerProduct (ips1 ips2 : InnerProductSpace) : InnerProductSpace where
  inner := fun v w => ips1.inner v w + ips2.inner v w
  inner_symm := fun v w => by
    dsimp
    rw [ips1.inner_symm v w, ips2.inner_symm v w]

def externalTensorProduct : String :=
  "External tensor product of forms on X and Y gives forms on X x Y."

def productHodgeStar : String :=
  "star_{XxY}(alpha x beta) = (-1)^{|alpha|*|beta|} star_X(alpha) x star_Y(beta)."

def s2xs2_betti : String := "S^2 x S^2: b_0=1, b_2=2, b_4=1, chi=4"

#eval "---- Products: additional ----"
#eval bettiProductFormula
#eval eulerProductFormula
#eval s2xs2_betti
