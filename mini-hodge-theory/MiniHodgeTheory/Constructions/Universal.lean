/-
# MiniHodgeTheory: Constructions -- Universal -- L3/L8
Universal coefficient theorem, Hodge decomposition.
-/
...
structure UniversalCoefficient where
  doc : String := "0 -> H^k x R -> H^k(.;R) -> Tor(H^{k-1},R) -> 0"

def harmonicUniversalProperty : String :=
  "Every de Rham cohomology class has a unique harmonic representative."

#eval "---- Constructions.Universal ----"
#eval harmonicUniversalProperty


def uct_over_field : String := "Over a field F: H^k(X;F) ~ Hom(H_k(X;F), F)."
def uct_over_Z : String := "0 -> H^k(X) x R -> H^k(X;R) -> Tor(H^{k-1}) -> 0 (exact)."
def uct_real : String := "For real coefficients, Tor(-,R) = 0, so H^k(X;R) ~ H_k(X;R)."
def hodgeSplittingExact : String := "0 -> im(d) -> Omega -> Har -> 0 is split exact."
def harmonicHomotopy : String := "Inclusion Har -> Omega and projection are homotopy inverses."
def abstractHodgeDecomp : String := "For any elliptic complex (E,d), E = Har + im(d) + im(d*)."
def boundaryHodgeTheory : String := "Absolute/relative boundary conditions for Hodge theory."

#eval "---- Universal: additional ----"
#eval uct_real
#eval abstractHodgeDecomp
