/-
# MiniHodgeTheory: Constructions -- Subobjects -- L3
Subcomplexes and invariant subspaces.
-/
...
structure Subcomplex (D : DiffOperator) where
  carrier : RealVec -> Prop
  closed_add : forall v w, carrier v -> carrier w -> carrier (RealVec.add v w)
  d_invariant : forall v, carrier v -> carrier (D.d.apply v)

#eval "---- Constructions.Subobjects ----"
#eval "Subcomplex defined"


def harmonicSubspace : String := "Har = ker(Delta) is a subcomplex since d(Har) = 0 and d*(Har) = 0."
def exactSubspace : String := "im(d) is a subcomplex since d(im(d)) = 0 (d^2 = 0)."
def coexactSubspace : String := "im(d*) is a subcomplex since d*(im(d*)) = 0 (d*^2 = 0)."
def hodgeSplitting : String := "Omega = Har + im(d) + im(d*) (orthogonal direct sum)."
def finiteDimSubcomplex : String := "If total space is finite-dim, then H is finite-dim."
def basicSubcomplex : String := "Forms with constant coefficients form a subcomplex."
def invariantSubcomplex : String := "G-invariant forms form a subcomplex for isometric G-actions."

#eval "---- Subobjects: additional ----"
#eval hodgeSplitting
