/-
# MiniHodgeTheory: Morphisms -- Equiv -- L2/L3
Chain homotopies and homotopy equivalences.
-/
...
structure ChainHomotopy {D1 D2 : DiffOperator} (f g : ChainMap D1 D2) where
  homotopy : LinearMap
  -- f - g = dY o h + h o dX

structure HomotopyEquiv (D1 D2 : DiffOperator) where
  forward : ChainMap D1 D2
  backward : ChainMap D2 D1
  -- forward o backward ~ id, backward o forward ~ id

def homotopyEquivId (D : DiffOperator) : HomotopyEquiv D D where
  forward := ChainMap.id D
  backward := ChainMap.id D

def homotopyInvariant : String :=
  "Homotopy equivalent complexes have isomorphic cohomology."

#eval "---- Morphisms.Equiv ----"
#eval homotopyInvariant
